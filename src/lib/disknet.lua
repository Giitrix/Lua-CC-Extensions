...
disknet.send(string channel, string message, optional number recipientID) Sends message on channel.
If a recipientID is specified, then only a disknet recipient whose computer
ID matches it will be able to receive the message.

disknet.receive(optional string channel, optional number senderID) Receives a disknet message.
If channel is specified, it will only accept messages from that channel.
If senderID is specified, it will only accept messages from that sender computer ID.
--]]
